  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/challonge/participants?tournamentId=${tournamentId}&participantId=${participantId}`,
        {
          method: 'DELETE',
        }
      );
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-300">
                    Apakah Anda yakin ingin menghapus peserta <span className="font-semibold">{participant?.name}</span>? Tindakan ini tidak dapat dibatalkan.
                  </p>
                </div>
